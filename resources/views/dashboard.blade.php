...
@section('content')
    ...
    <div class="min-vh-100 pb-5">
            ...
            <div class="container-fluid">
                ...
                <div class="d-flex align-items-center ms-auto me-4">
                    <a href="{{ route('dashboard') }}"
                        class="menu-link {{ request()->routeIs('dashboard') ? 'active' : '' }}">Home</a>
        ...
        <div class="container mt-5 p-4 rounded shadow" style="background:#9FB6C7;">
            ...
            {{-- SEARCH --}}
            <form method="GET" action="{{ route('dashboard') }}" class="d-flex flex-wrap align-items-center gap-3 mb-4">
                <div class="d-flex flex-wrap align-items-center gap-3 mb-4">
                    ...
                    <button type="submit" name="status" value=""
                        class="filter-btn {{ request('status') == '' ? 'active' : '' }}">Semua</button>
                    <button type="submit" name="status" value="hilang"
                        class="filter-btn {{ request('status') == 'hilang' ? 'active' : '' }}">Barang Hilang</button>
                    <button type="submit" name="status" value="ditemukan"
                        class="filter-btn {{ request('status') == 'ditemukan' ? 'active' : '' }}">Barang Ditemukan</button>
                </div>
                <div class="d-flex flex-wrap align-items-center gap-3 mb-4">
                    <div>
                    ...
                    <div class="align-self-end">
                        <a href="{{ route('dashboard') }}" class="btn btn-secondary rounded-pill px-4">
                    ...
                    @foreach ($items as $item)
                        <div class="item-card position-relative">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <p class="mb-1"><i class="fa-solid fa-user"></i> Dilaporkan oleh:
                                    {{ $item->user->name }}</p>
                                <p class="mb-1"><i class="fa-solid fa-location-dot"></i> Lokasi: {{ $item->lokasi }}
                                </p>
                                <p class="mb-1"><i class="fa-solid fa-calendar"></i> Tanggal:
                            ...
                            <div class="text-end">
                                ...
                                @elseif($item->status === 'diklaim')
                                    <div
                                        class="d-flex align-items-center justify-content-end gap-2 text-secondary fw-bold">
                                        <div class="status-dot" style="border:2px solid gray;"></div> DIKLAIM
                                    </div>
                                @endif

                                {{-- pastikan route-nya ada --}}
                                <a href="{{ route('items.show_item', $item->id) }}" class="detail-btn mt-2">Lihat
                                    Detail</a>
                            </div>
                        </div>
                    @endforeach
